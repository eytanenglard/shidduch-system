...
exports.getServerStatusInfo = getServerStatusInfo;
exports.getUrgentSuggestions = getUrgentSuggestions;
exports.getSuggestionsByCategory = getSuggestionsByCategory;
exports.calculateUserStats = calculateUserStats;
exports.isUserInActiveProcess = isUserInActiveProcess;
exports.getPersonalizedStatusMessage = getPersonalizedStatusMessage;
/**
 * מחזיר מידע על סטטוס ההצעה מצד השרת
 */
function getServerStatusInfo(status, firstPartyId, secondPartyId, currentUserId) {
 ...
 */
function getUrgentSuggestions(suggestions, userId) {
    return suggestions.filter(suggestion => {
        const statusInfo = getServerStatusInfo(suggestion.status, suggestion.firstPartyId, suggestion.secondPartyId, userId);
        // בדיקה אם המשתמש צריך לפעול
        if (!statusInfo.requiresUserAction)
            return false;
 ...
 */
function getSuggestionsByCategory(suggestions, category, userId) {
    return suggestions.filter(suggestion => {
        const statusInfo = getServerStatusInfo(suggestion.status, suggestion.firstPartyId, suggestion.secondPartyId, userId);
 ...
 */
function calculateUserStats(suggestions, userId) {
 ...
 */
function isUserInActiveProcess(suggestions, userId) {
    ...
    return suggestions.some(suggestion => (suggestion.firstPartyId === userId || suggestion.secondPartyId === userId) &&
        activeStatuses.includes(suggestion.status));
}
/**
 * מחזיר הודעת סטטוס מותאמת למשתמש
 */
function getPersonalizedStatusMessage(status, isFirstParty, targetPartyName) {
    const messages = {
        PENDING_FIRST_PARTY: {
