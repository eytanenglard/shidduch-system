...
exports.analyzePairCompatibility = analyzePairCompatibility;
exports.getProfileAnalysis = getProfileAnalysis;
exports.analyzeSuggestionForUser = analyzeSuggestionForUser;
exports.generateSuggestionRationale = generateSuggestionRationale;
exports.generateFullSuggestionRationale = generateFullSuggestionRationale;
exports.generateNeshamaTechSummary = generateNeshamaTechSummary;
 ...
 */
async function analyzePairCompatibility(profileAText, profileBText, language = 'he') {
 ...
 * @returns Promise שמחזיר אובייקט ניתוח מובנה, או null במקרה של כישלון.
 */
async function getProfileAnalysis(userNarrativeProfile, language = 'he') {
  ...
  `;
    try {
        ...
        try {
            ...
            console.log('--- [AI Profile Advisor] Successfully received and parsed analysis from Gemini API. ---');
            return parsedJson;
        }
        catch (parseError) {
            // שלב 3: אם הפענוח נכשל, הדפס שגיאה מפורטת ואת התשובה הגולמית לטובת דיבאגינג.
            console.error('[AI Profile Advisor] Failed to parse JSON response from Gemini.', parseError);
        ...
        console.error('[AI Profile Advisor] Error during profile analysis process:', error);
        return null;
    }
}
/**
 ...
 */
async function analyzeSuggestionForUser(currentUserProfileText, suggestedUserProfileText) {
 ...
 */
async function generateSuggestionRationale(profile1Text, profile2Text) {
 ...
 */
async function generateFullSuggestionRationale(profile1Text, profile2Text) {
 ...
 */
async function generateNeshamaTechSummary(userNarrativeProfile, locale = 'he') {
    console.log(`--- [AI NeshamaTech Summary] Starting DYNAMIC summary generation for locale: ${locale} ---`);
    if (!userNarrativeProfile) {
        console.error('[AI NeshamaTech Summary] Called with an empty user narrative profile.');
        return null;
    }
    const model = genAI.getGenerativeModel({
        model: 'gemini-2.5-flash',
  ...
  `;
    try {
        ...
        const jsonString = response.text();
        if (!jsonString) {
            console.error(`[AI NeshamaTech Summary] Gemini API returned an empty response for locale: ${locale}.`);
            return null;
        }
        ...
        try {
            ...
            console.log(`--- [AI NeshamaTech Summary] Successfully received and parsed summary from Gemini API for locale: ${locale}. ---`);
            return parsedJson;
        }
        catch (parseError) {
            console.error(`[AI NeshamaTech Summary] Failed to parse JSON response for locale: ${locale}.`, parseError);
            console.error('--- RAW AI RESPONSE THAT FAILED PARSING ---');
            console.error(jsonString);
            console.error('--- END OF RAW AI RESPONSE ---');
            throw new Error('Invalid JSON response from AI service.');
        ...
        console.error(`[AI NeshamaTech Summary] Error during summary generation for locale: ${locale}:`, error);
        return null;
    }
}
