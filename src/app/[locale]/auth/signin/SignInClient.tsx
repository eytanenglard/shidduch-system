...
const AppleLogo: React.FC<{ className?: string }> = ({
  className = 'h-5 w-5',
}) => (
  <svg
    className={className}
...
// ============================================================================
// LIGHTWEIGHT BACKGROUND — reduced from 4 blobs to 2 + simplified SVG
// ============================================================================

const DynamicBackground: React.FC = () => (
  <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
    <motion.div
      className="absolute top-10 left-10 w-96 h-96 bg-gradient-to-br from-teal-400/15 to-emerald-500/15 rounded-full blur-3xl"
      animate={{ y: [0, -30, 0], scale: [1, 1.1, 1] }}
      transition={{ duration: 15, repeat: Infinity, ease: 'easeInOut' }}
    />
    <motion.div
      className="absolute bottom-20 right-10 w-80 h-80 bg-gradient-to-br from-orange-400/15 to-amber-500/15 rounded-full blur-3xl"
...
export default function SignInClient({ dict, locale }: SignInClientProps) {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session) {
      const redirectUrl = (session as any)?.redirectUrl || `/${locale}/profile`;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (result?.error) {
      ...
      // Activate cooldown after MAX_FAILED_ATTEMPTS
      if (newFailedCount >= MAX_FAILED_ATTEMPTS_BEFORE_COOLDOWN) {
        setCooldownRemaining(COOLDOWN_SECONDS);
        setFailedAttempts(0);
      }
    } else {
      // Success — loading state will persist until the session effect triggers redirect
      // No need to setIsLoading(false) here — the page will navigate away
      setFailedAttempts(0);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError('');
    await signIn('google', { callbackUrl }, { hl: locale });
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 relative z-10">
          ...
          <motion.div variants={itemVariants} className="mb-4">
            <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded-full px-6 py-3 shadow-lg border border-white/60 mb-6">
              ...
              <span className="text-sm font-medium text-gray-700">
                {dict.welcomeBack || (isRTL ? 'ברוכים השבים' : 'Welcome Back')}
              </span>
        ...
        <motion.div
          ...
          animate={
            shouldShake
              ? { opacity: 1, y: 0, x: [-6, 6, -6, 6, 0] }
              : { opacity: 1, y: 0 }
          }
          transition={
            shouldShake ? { duration: 0.4 } : { duration: 0.5, delay: 0.3 }
          }
          className="w-full max-w-md"
        >
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/60 overflow-hidden relative">
            ...
            <div className="relative z-10 p-6 sm:p-8">
                  ...
                  >
                    <div className="p-4 bg-amber-50 border-2 border-amber-200 rounded-2xl text-center">
                      <p className="text-amber-800 text-sm font-medium">
                        {isRTL
                          ? `יותר מדי ניסיונות. נסה שוב בעוד ${cooldownRemaining} שניות.`
              ...
              <form onSubmit={handleSubmit} className="space-y-5 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <div
                      className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 transition-colors ${email ? 'text-teal-500' : 'text-gray-400'}`}
                    >
                    ...
                    <input
                      ...
                      className={`w-full ${isRTL ? 'pr-11 pl-4' : 'pl-11 pr-4'} py-3.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-200 focus:border-teal-400 focus:outline-none transition-colors bg-white/50 hover:border-gray-300 text-gray-800 placeholder:text-gray-400 touch-manipulation`}
                      placeholder={dict.emailPlaceholder}
                      required
                      disabled={isAnyLoading || isCoolingDown}
                      dir={isRTL ? 'rtl' : 'ltr'}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <div
                      className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 transform -translate-y-1/2 transition-colors ${password ? 'text-teal-500' : 'text-gray-400'}`}
                    >
                    ...
                    <input
                      ...
                      className={`w-full ${isRTL ? 'pr-11 pl-12' : 'pl-11 pr-12'} py-3.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-200 focus:border-teal-400 focus:outline-none transition-colors bg-white/50 hover:border-gray-300 text-gray-800 placeholder:text-gray-400 touch-manipulation`}
                  ...
                  >
                    <Link
                      ...
                      className="text-sm text-teal-600 hover:text-teal-700 hover:underline font-medium transition-colors touch-manipulation"
                    >
              ...
              <div className="space-y-3">
                ...
                <Button
                  ...
                  disabled={isAnyLoading || isCoolingDown}
                  className="w-full py-4 rounded-xl flex items-center justify-center gap-3 transition-all duration-200 bg-black hover:bg-gray-900 text-white shadow-lg hover:shadow-xl touch-manipulation active:scale-[0.98] border-0"
                >
                  {isAppleLoading ? (
                    <>
                      <Loader2 className="animate-spin h-5 w-5" />
                      <span className="font-medium">
                        {dict.appleButtonLoading ||
                          (isRTL
                            ? 'מתחבר עם Apple...'
                    ...
                    <>
                      ...
                      <span className="font-medium">
                        {dict.appleButton ||
                          (isRTL ? 'התחברות עם Apple' : 'Sign in with Apple')}
                      </span>
                    </>
                  )}
                </Button>
                ...
                <Button
                  ...
                  disabled={isAnyLoading || isCoolingDown}
                  variant="outline"
                  className="w-full border-2 border-gray-200 hover:border-gray-300 hover:bg-gray-50 py-4 rounded-xl flex items-center justify-center gap-3 transition-all duration-200 bg-white/50 touch-manipulation active:scale-[0.98]"
                >
                  {isGoogleLoading ? (
                    <>
                      <Loader2 className="animate-spin h-5 w-5" />
              ...
              <div className="mt-8 text-center">
                <p className="text-gray-600 text-sm">
                  ...
                  <Link
                    ...
                    className="text-teal-600 font-semibold hover:text-teal-700 hover:underline transition-colors inline-flex items-center gap-1 touch-manipulation"
                  >
                    {dict.signUpLink}
                    {isRTL ? (
