...
// ============================================================================
// 🍎 Apple Private Key Helper
// Apple requires a private key in PEM format for generating the client secret.
// The key is stored in .env as a single line with \n escape sequences.
// ============================================================================
function getApplePrivateKey(): string {
  const key = process.env.APPLE_PRIVATE_KEY;
  if (!key) {
    throw new Error("APPLE_PRIVATE_KEY is not set in environment variables");
  }
  // Replace literal \n with actual newlines (needed when stored in .env)
  return key.replace(/\\n/g, '\n');
}
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    // ========================================================================
    AppleProvider({
      clientId: process.env.APPLE_CLIENT_ID!,       // Service ID (e.g., "com.neshamatech.web")
      clientSecret: process.env.APPLE_CLIENT_SECRET!, // Generated JWT or auto-generated by next-auth
      authorization: {
        params: {
          scope: "name email",
      ...
      profile(profile) {
        ...
        // Apple only sends name on FIRST sign-in, so it may be empty
        ...
        const userForAdapter: ExtendedUser = {
          ...
          phone: null,
          image: null, // Apple doesn't provide a profile picture
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
        ...
        try {
          ...
          const createdDbUser = await prisma.user.create({
            data: {
              ...
              appleId: account.providerAccountId, // Store Apple's sub as appleId
              role: UserRole.CANDIDATE,
              status: UserStatus.PENDING_PHONE_VERIFICATION,
              ...
              // Create skeleton profile (same as mobile flow)
              profile: {
                create: {
                  availabilityStatus: 'AVAILABLE',
                  isProfileVisible: false,
                  gender: 'FEMALE', // Temporary value
                  birthDate: new Date('2000-01-01T00:00:00.000Z'), // Temporary
      ...
      if (account?.provider === 'apple' && !dbUser.appleId) {
        ...
        // Also link the account record if it doesn't exist
        const existingAccount = await prisma.account.findUnique({
          where: {
            provider_providerAccountId: {
              provider: account.provider,
