...
import { ArrowLeft, Heart, Shield, User,Lightbulb, Users, Sparkles, Brain, Handshake, ChevronDown } from "lucide-react"; 
  ...
  return (
    <section className="relative min-h-screen pt-20 pb-16 md:pt-24 md:pb-20 overflow-hidden flex flex-col items-center justify-center w-full px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col items-center">
        ...
        >
          {/* ========================================================== */}
          {/* ||      גרסה סופית ומתוקנת למובייל (אנכית)           || */}
          {/* ========================================================== */}
          {/* שינוי 1: הגדלנו את הרווח הכללי ל-gap-3 */}
          <div className="md:hidden flex flex-col items-center gap-3"> 
            {/* AI Technology */}
            <div className={`flex flex-col items-center gap-2 opacity-0 ${isVisible ? 'animate-fade-in-up' : ''}`} style={{ animationDelay: '200ms' }}>
            ...
            {/* Dashed line */}
            <div className={`h-8 w-px border-r border-dashed border-gray-400 opacity-0 ${isVisible ? 'animate-fade-in-up' : ''}`} style={{ animationDelay: '400ms' }} />

            {/* Human Touch */}
            <div className={`flex flex-col items-center gap-2 opacity-0 ${isVisible ? 'animate-fade-in-up' : ''}`} style={{ animationDelay: '600ms' }}>
              <div className="p-4 bg-white/60 backdrop-blur-md rounded-full shadow-lg border border-white/50">
            ...
            {/* "Leads to..." Icon */}
            {/* שינוי 2: הגדלנו את הרווח האנכי של החץ ל-my-2 */}
            <div className={`opacity-0 ${isVisible ? 'animate-fade-in-up' : ''}`} style={{ animationDelay: '800ms' }}>
                <ChevronDown className="w-7 h-7 text-gray-400 my-2"/>
            </div>

             {/* The Result: Match Point */}
             {/* שינוי 3: החלפנו לאנימציה החדשה (ללא transform) והסרנו את ה-margin */}
             <div className={`opacity-0 ${isVisible ? 'animate-mobile-match-point' : ''}`} style={{ animationDelay: '1000ms' }}>
          ...
          {/* ========================================================== */}
          {/* ||        גרסת הדסקטופ המקורית (מוסתרת במובייל)      || */}
          {/* ========================================================== */}
...
{/* --- 3. Stats Bar (Social Proof) - מסודר לפי צבעים --- */}
<div
...
>
  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 bg-white/60 backdrop-blur-md py-4 px-6 rounded-2xl shadow-lg border border-white/50">
    ...
    {/* ======================================================== */}
    {/* ||      שורה 1: תכלת | ורוד | תכלת                    || */}
    {/* ||      (הבטחת הבסיס: פרטיות, טכנולוגיה, ליווי)      || */}
    {/* ======================================================== */}
    ...
    {/* ======================================================== */}
    {/* ||      שורה 2: ורוד | תכלת | ורוד                    || */}
    {/* ||      (הבידול הייחודי: תהליך, זהות, איכות)         || */}
    {/* ======================================================== */}
    <StatItem icon={<Lightbulb className="h-5 w-5 text-pink-500" />} label="שידוך מנומק" value="AI + שדכן" color="pink" />
