...
import AnalysisResultDisplay from '@/app/[locale]/(authenticated)/profile/components/advisor/AnalysisResultDisplay';
...
export const AiMatchmakerProfileAdvisorDialog: React.FC<AiMatchmakerProfileAdvisorDialogProps> = ({
  candidate,
  isOpen,
  onClose,
  dict,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl w-[95vw] h-[90vh] flex flex-col p-0" dir={direction}>
        <DialogHeader className="p-4 border-b">
          <div className="flex justify-between items-center">
             <DialogTitle className="flex items-center gap-2 text-xl">
                <Bot className="w-6 h-6 text-indigo-500" />
                <span>{dict.dialogTitle} {candidate?.firstName} {candidate?.lastName}</span>
              </DialogTitle>
            <DialogClose asChild>
        ...
        <div className="flex-grow overflow-y-auto p-4 md:p-6 bg-slate-50/50">
          {isLoading && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-lg font-semibold text-gray-700">{dict.loadingTitle}</p>
              <p className="text-sm text-gray-500 mt-2">{dict.loadingDescription}</p>
            </div>
          )}
          {error && (
            <div className="flex flex-col items-center justify-center h-full text-center">
