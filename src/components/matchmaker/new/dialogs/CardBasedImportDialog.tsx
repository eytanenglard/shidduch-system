...
const BUILTIN_SOURCE_OPTIONS: SourceOption[] = [
  {
    id: 'shvivel',
...
export const CardBasedImportDialog: React.FC<CardBasedImportDialogProps> = ({
  ...
  // =========================================================================
  // AI Preparation — runs after save to generate metrics, vectors, tags
  // =========================================================================
  ...
  const saveCard = useCallback(
    async (cardId: string): Promise<boolean> => {
      ...
      try {
        ...
        updateCard(cardId, {
          ...
          aiPrepStatus: candidateId ? 'preparing' : 'idle',
        });
        setSavedCount((prev) => prev + 1);
        toast.success(`${fields.firstName} ${fields.lastName} נשמר/ה!`);

        // Trigger full AI preparation in background
        if (candidateId) {
    ...
    [cards, referredByGlobal, updateCard, prepareCardAi]
  );
...
const PERSONALITY_TYPE_LABELS: Record<string, string> = {
  LEADER: 'מנהיג/ה',
  SUPPORTER: 'תומך/ת',
  ANALYTICAL: 'אנליטי/ת',
  CREATIVE: 'יצירתי/ת',
...
const HUMOR_STYLE_LABELS: Record<string, string> = {
  CYNICAL: 'ציני',
  LIGHT: 'קליל',
...
const AiPrepSection: React.FC<AiPrepSectionProps> = ({ status, result, error, onRetry }) => {
